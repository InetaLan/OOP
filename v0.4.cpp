...
void read_from_file(vector<studentas>& Eil, int k)
{
...
int main()
{
	...
	for (int j = 0; j < k; j++)
	{
		...
		{
			moksliukai.push_back(studentai.at(j));
			moks++;
		}
	}
	...
	for (int i = 0; i < k; i++)
	{
		if (studentai.at(i).GP < paz)
		{
			varg_failas << studentai.at(i).Vard << setw(20) << studentai.at(i).Pav << setw(18) << studentai.at(i).GP << endl;
		}
	...
	for (int j = 0; j < k; j++)
	{
		if (studentai.at(j).GP >= paz)
		{
			moks_failas << studentai.at(j).Vard << setw(20) << studentai.at(j).Pav << setw(18) << studentai.at(j).GP << endl;
		}
	}
